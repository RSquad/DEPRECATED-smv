...
export default async ({
  ...
  const reclaimDepositResult = await userWallet.call({
  ...
  const msg = await waitForMessage(
    client,
    {
      ...
      created_at: { ge: reclaimDepositResult.transaction.now },
    },
    "value id"
  );
  expect(parseInt(msg.value, 16)).to.be.greaterThan(_reclaimAmount * 1e9 - 0.5e9);
